...
\textbf{True Utility:}
The optimal policy moves toward (10,10) with Manhattan distance $(10-x) + (10-y)$. With step cost:
\[ U(x,y) = 1 - \gamma[(10-x)+(10-y)] = 1 - \gamma(20 - x - y) \]

Expanding:
\[ U(x,y) = (1 - 20\gamma) + \gamma x + \gamma y \]


\textbf{Linear Approximation:}
The true utility function IS linear in $x$ and $y$:
\[ \hat{U}(x,y) = \theta_0 + \theta_1 x + \theta_2 y \]
where $\theta_0 = 1 - 20\gamma$, $\theta_1 = \gamma$, $\theta_2 = \gamma$.

\textbf{Quality:} Perfect fit. The linear approximation exactly matches the true utility.
...
\begin{itemize}
    \item $\phi_1$: Bias term (always 1)
    \item $\phi_2$: Indicator that action moves toward (4,3) in x-direction
    \item $\phi_3$: Indicator that action moves toward (4,3) in y-direction
    \item $\phi_4$: Normalized negative distance to goal (4,3)
    \item $\phi_5$: Normalized distance from bad state (4,2)
    \item $\phi_6, \phi_7, \phi_8$: Action preference indicators
\end{itemize}

\subsubsection*{2. REINFORCE Algorithm}
REINFORCE is a Monte Carlo policy gradient method that directly updates policy parameters based on observed returns.

\begin{algorithm}[H]
\caption{REINFORCE Algorithm with Baseline}
\begin{algorithmic}[1]
\State Initialize policy parameters $\theta$
\State Initialize baseline $b(s)$ for each state
\For{each episode}
    ...
    \For{each step $t$ in trajectory}
        ...
        \State Update baseline: $b(s_t) \leftarrow b(s_t) + \frac{1}{N(s_t)}(G_t - b(s_t))$
        \State $\delta \leftarrow G_t - b(s_t)$ \Comment{Advantage}
...
PEGASUS (Policy Evaluation by Gradient And Search Using Scenarios) uses \textbf{correlated sampling} to reduce variance.

\textbf{Key Idea:} Fix a set of random seeds $\{u_1, u_2, \ldots, u_m\}$ that determine \textit{both} the stochastic action selection \textit{and} the stochastic environment transitions. When comparing different policies, use the same seeds so the only difference in outcomes is due to the policy change, not random variation.

\begin{algorithm}[H]
\caption{PEGASUS Algorithm}
\begin{algorithmic}[1]
\State Fix random seeds $U = \{(u_1^a, u_1^e), \ldots, (u_m^a, u_m^e)\}$ for actions and environment
\State Initialize policy parameters $\theta$ with small random values
\Repeat
    \State $\rho(\theta) \leftarrow \frac{1}{m} \sum_{i=1}^{m} \text{SimulateWithSeeds}(\pi_\theta, u_i^a, u_i^e)$
    \State Estimate gradient $\nabla_\theta \rho(\theta)$ using finite differences:
    \State \quad For each parameter $\theta_j$:
    \State \quad \quad $\frac{\partial \rho}{\partial \theta_j} \approx \frac{\rho(\theta + \epsilon e_j) - \rho(\theta - \epsilon e_j)}{2\epsilon}$
    \State $\theta \leftarrow \theta + \alpha \nabla_\theta \rho(\theta)$
\Until{convergence}
\end{algorithmic}
\end{algorithm}

\textbf{CRITICAL Implementation Note:} The key to PEGASUS working correctly is that we must use \textbf{stochastic action sampling} (not greedy/deterministic) during gradient estimation, but with fixed random seeds for reproducibility. This allows the finite difference method to detect how policy parameter changes affect expected return.
...
\hline
Variance & High & Low (due to correlated samples) \\
Samples per update & Many episodes & Fewer scenarios \\
Bias & Unbiased & Small bias (finite diff.) \\
Convergence speed & Slow & Faster \\
Memory requirement & Low & Higher (stores seeds) \\
Gradient estimation & Direct (analytical) & Indirect (finite difference) \\
\hline
\end{tabular}
...
\begin{itemize}
    \item \textbf{REINFORCE:} Requires many episodes due to high variance. The stochastic nature of the environment (0.8 intended, 0.1 each perpendicular direction) causes significant variability in returns, making gradient estimates noisy.
    \item \textbf{PEGASUS:} By fixing randomness across policy comparisons, we can reliably detect small improvements in the policy. This dramatically reduces variance and speeds up learning.
    \item \textbf{Common Ground:} Both converge to optimal or near-optimal policies for simple environments like the 4×3 world.
    \item \textbf{Implementation Issue:} If PEGASUS uses deterministic action selection (greedy), it fails to learn because the finite difference gradient becomes zero. The policy must sample actions stochastically.
\end{itemize}
